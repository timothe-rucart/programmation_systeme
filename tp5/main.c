...
int main(int argc, char *argv[]){
	...
	int fc;
  	...
  	while(i<argc && argv[i][0]=='-'){
    	...
    	while(argv[i][j]!='\0') {
     		...
     		if(argv[i][j] != 'l' && argv[i][j] != 'w' && argv[i][j] != 'c'){
     			printf("%c\n",argv[i][j] );
				printf("L'argument saisie n'est pas disponible: '%s' \n", argv[i]);
				exit(1);
      		}
  	...
  	//Cas ou aucun argument n'a ete saisie
 	if(opL==0 && opW ==0 && opC==0){
  	...
  	//Traitement de l'affichage en entree standard si aucun fichier n'a été passé
	if (i == argc) {
    ...
    //Traitement du fichier passe en argument
    while(i<argc){
	...
	/*n = open("cac.txt",O_RDONLY) ;		// renvoie -1 s'il n'existe pas
